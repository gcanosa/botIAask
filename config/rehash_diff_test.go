...
func TestRehashDiff_IRCChannels(t *testing.T) {
	f := false
	before := &Config{
		IRC: IRCConfig{Networks: []IRCNetworkConfig{{Name: "net1", Channels: []IRChannel{
			{Name: "#a"},
			{Name: "#b", AutoJoin: &f},
		}}}},
...
func TestRehashDiff_IRCEndpoint(t *testing.T) {
	before := &Config{
		IRC:     IRCConfig{Networks: []IRCNetworkConfig{{Name: "n1", Server: "a.net", Port: 6697, Nickname: "B1", UseSSL: true}}},
		Stats:   StatsConfig{Interval: 60},
		Uploads: UploadsConfig{MaxFileMB: 200},
	}
	after := &Config{
		IRC:     IRCConfig{Networks: []IRCNetworkConfig{{Name: "n1", Server: "b.net", Port: 6697, Nickname: "B1", UseSSL: true}}},
		Stats:   StatsConfig{Interval: 60},
		Uploads: UploadsConfig{MaxFileMB: 200},
	}
	lines := RehashDiff(before, after)
	if !hasLineContaining(lines, "reconnecting") {
		t.Fatalf("expected reconnect note, got %q", lines)
	}
}

func TestCloneConfig(t *testing.T) {
	orig := &Config{IRC: IRCConfig{Networks: []IRCNetworkConfig{{Name: "n1", Channels: []IRChannel{{Name: "#x"}}}}}, Stats: StatsConfig{Interval: 60}, Uploads: UploadsConfig{MaxFileMB: 200}}
	c, err := CloneConfig(orig)
	if err != nil {
		t.Fatal(err)
	}
	c.IRC.Networks[0].Channels[0].Name = "#y"
	if orig.IRC.Networks[0].Channels[0].Name != "#x" {
